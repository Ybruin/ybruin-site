(function (global) {
    ...
    rproto.push = function () {
        ...
        each(args, function (arg) {
            var id = scrat.alias(arg),
                type = fileType(id),
                res = scrat.get(id),
                isComponents = fileComponents(id);

            if(type === 'css' && isComponents){
                type = 'comcss';
            }else if(type === 'js' && isComponents){
                type = 'comjs';
            }
            ...
            if (((type === 'css' || type === 'comcss')&& !res.rawCSS && !res.parsed) ||
                ((type === 'js' || type === 'comjs')&& !res.factory && !res.exports)) {
    ...
    rproto.run = function () {
        ...
        function resourceCombo (resdeps,isComponents) {
            var urlLength = 0,
                ids = [],
                deps = [];
            each(resdeps, function (res, i) {
                ...
                if (urlLength + res.id.length < options.maxUrlLength) {
                    ...
                    scrat.load(that.genUrl(ids,isComponents), onload, onload);
                    urlLength = res.id.length;
                    ids = [res.id];
                    deps = [res];
                }
        ...
        if (combo) {
            if (cache) {
                ...
                .concat(depends.comcss || [],true).concat(depends.comjs || []),true);
            } else {
                resourceCombo(depends.css || []);
                resourceCombo(depends.js || []);
                resourceCombo(depends.comcss || [],true);
                resourceCombo(depends.comjs || [],true);
            }
        } else {
            each((depends.css || []).concat(depends.js || []).concat(depends.comcss || [],true).concat(depends.comjs || [],true), function (res) {
    ...
    rproto.genUrl = function (ids,isComponents) {
        console.log(ids)
        if (type(ids) === 'string') ids = [ids];

        var options = scrat.options,
            url = isComponents == true ? (options.combo && options.componentsPattern || options.comboPattern) : (options.combo && options.comboPattern || options.urlPattern);
    ...
    var TYPE_CO = /(components)\/.+\.(js|css)/i;
    function fileComponents(str) {
        var ext = false;
        str.replace(TYPE_CO, function (m, $1) {
            if($1 == 'components'){
                ext = true;
            }
        });
